...
function unlock(){if(ready||saved||destroyed)return;ready=true;btn.disabled=false;btn.textContent='Record obtained finding';msg.className='svc-message';msg.textContent='Assessment accepted. Record the finding when ready.'}
function tryUnlock(){setTimeout(()=>{if(destroyed||saved)return;if(!def.success||def.success())unlock();else{msg.className='svc-message';msg.textContent='Review the feedback and complete the assessment correctly before recording.'}},30)}
