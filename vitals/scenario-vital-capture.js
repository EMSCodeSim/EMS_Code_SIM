...
let ready=false,saved=!!api.active()?.findings?.[def.key],partnerTimer=null,partnerBusy=false;
const host=document.createElement('section');host.id='scenarioVitalCapture';host.innerHTML=`<div class="svc-card"><h2>${def.label}: obtain the finding</h2><p>${def.prompt}</p><div class="svc-actions"><button class="svc-record" type="button" ${saved?'disabled':''}>${saved?'Finding already recorded':'Record obtained finding'}</button><button class="svc-partner" type="button" ${saved?'disabled':''}>Assign to partner</button></div><div class="svc-partner-status" hidden><div class="svc-partner-row"><strong>Partner obtaining ${def.label.toLowerCase()}</strong><span class="svc-countdown"></span></div><div class="svc-progress"><span></span></div><button class="svc-cancel" type="button">Cancel assignment</button></div><div class="svc-message" aria-live="polite"></div></div>`;document.body.appendChild(host);
const btn=host.querySelector('.svc-record'),partnerBtn=host.querySelector('.svc-partner'),partnerStatus=host.querySelector('.svc-partner-status'),countdown=host.querySelector('.svc-countdown'),progress=host.querySelector('.svc-progress span'),cancelBtn=host.querySelector('.svc-cancel'),msg=host.querySelector('.svc-message');if(!saved)btn.disabled=true;
function unlock(){if(ready)return;ready=true;stopResultObserver();if(!saved){btn.disabled=false;btn.textContent='Record obtained finding'}}
...
// Avoid observing the entire document. Animated simulators update text frequently,
// and scanning document.body.innerText on every mutation can freeze the page.
let resultObserver=null;
const resultSelectors=['#result','#feedback','#status','#reading','#resultText','.result','.feedback','.status'];
function stopResultObserver(){
  if(resultObserver){resultObserver.disconnect();resultObserver=null;}
}
function inspectResultNode(node){
  if(ready||saved||!node)return;
  const text=(node.textContent||'').trim();
  if(/correct|result ready|observation complete|actual finding|reading complete|assessment complete|\/3 correct/i.test(text)){
    unlock();
    stopResultObserver();
  }
}
const resultNodes=[...new Set(resultSelectors.flatMap(sel=>[...document.querySelectorAll(sel)]))];
if(resultNodes.length){
  resultObserver=new MutationObserver(records=>{
    for(const record of records){
      inspectResultNode(record.target.nodeType===Node.TEXT_NODE?record.target.parentElement:record.target);
      if(ready)break;
    }
  });
  resultNodes.forEach(node=>{
    resultObserver.observe(node,{subtree:true,childList:true,characterData:true});
    inspectResultNode(node);
  });
}
...
function saveFinding(source='scenario-simulator'){
 ...
 saved=true;partnerBusy=false;stopResultObserver();clearInterval(partnerTimer);btn.disabled=true;partnerBtn.disabled=true;btn.textContent='Finding recorded';partnerStatus.hidden=true;msg.className='svc-message svc-saved';msg.textContent=source==='partner-assignment'?`Your partner reports ${def.label.toLowerCase()}: ${value}. It was added to the patient findings.`:`${def.label} added to this patient’s findings.`;const detail={assessment:def.key,label:def.label,finding:value,normality,source};window.dispatchEvent(new CustomEvent('emscodesim:assessment-saved',{detail}));setTimeout(()=>window.EMSCodeSimScenarioFlow?.show?.(),0);
}
btn.addEventListener('click',()=>{if(!ready||saved||partnerBusy)return;saveFinding()});
partnerBtn.addEventListener('click',()=>{
 if(saved||partnerBusy)return;partnerBusy=true;btn.disabled=true;partnerBtn.disabled=true;partnerStatus.hidden=false;msg.className='svc-message';msg.textContent='Continue assessing the patient while your partner gathers this information.';
 const total=Math.max(5,Number(def.delay)||15);let remaining=total;countdown.textContent=`${remaining}s`;progress.style.width='0%';
 partnerTimer=setInterval(()=>{remaining-=1;countdown.textContent=`${Math.max(remaining,0)}s`;progress.style.width=`${Math.min(100,((total-remaining)/total)*100)}%`;if(remaining<=0){clearInterval(partnerTimer);saveFinding('partner-assignment')}},1000);
});
cancelBtn.addEventListener('click',()=>{if(!partnerBusy||saved)return;clearInterval(partnerTimer);partnerBusy=false;partnerStatus.hidden=true;partnerBtn.disabled=false;btn.disabled=!ready;msg.textContent='Partner assignment canceled. You can perform the skill or assign it again.'});
})();

window.addEventListener('pagehide',()=>{stopResultObserver();clearInterval(partnerTimer);},{once:true});
