...
function newCase(){ let next = window.EMSCodeSimScenarioRuntime?.chooseCase('motor_sensory', cases, state.current) || cases[0];state.current=next; $('caseTitle').textContent=next.title; $('caseDescription').textContent=next.description; resetForm(); }
