(() => {
  ...
  const PROFILES = {
    ...
    horse_crush: {
      responder:'Patient',
      communication:'Linda is alert and oriented, answers clearly, and avoids moving her left leg.',
      opening:'Linda looks at you and says, “My left hip really hurts. Please don’t move or straighten this leg.”',
      fallback:'“I’m not sure. Please don’t move my left leg.”',
      repeatPrefix:'Like I said, “',
      sampleRequired:['symptoms','allergies','medications','medical_history','last_intake','events'],
      opqrstRequired:['onset','provocation','quality','radiation','severity','time'],
      ...
      questions:[
        {id:'name',category:'identity',label:'Preferred name',prompt:'What name would you like us to use?',response:'Linda is fine.',keywords:['name','preferred name','who are you','identify']},
        {id:'dob',category:'identity',label:'Approximate birth year',prompt:'About what year were you born?',response:'I was born in 1962.',keywords:['birth year','date of birth','dob','birthday','born']},
        {id:'age',category:'identity',label:'Approximate age',prompt:'About how old are you?',response:'I’m 64.',keywords:['age','how old','approximate age']},
        {id:'address',category:'identity',label:'General residence',prompt:'Do you live locally?',response:'I live on a small place outside town with my husband, Ray.',keywords:['address','home','where do you live','local','residence']},
        {id:'emergency_contact',category:'identity',label:'Support person',prompt:'Is there a family member or support person who should know you are being transported?',response:'Please contact my husband, Ray. He should be nearby.',keywords:['emergency contact','contact','family','support person','call someone']},
        {id:'chief_complaint',category:'current',label:'What hurts the most?',prompt:'What hurts the most right now?',response:'My left hip hurts the most. Please don’t straighten or lower this leg.',keywords:['hurt','chief complaint','pain','where']},
        {id:'symptoms',category:'current',label:'What symptoms are you having?',prompt:'What symptoms are you having?',response:'The main problem is severe pain in my left hip that runs down my leg. I’m not having chest pain, trouble breathing, stomach pain, neck pain, or back pain.',keywords:['symptom','chest pain','shortness','abdomen','neck','back']},
        {id:'onset',category:'opqrst',label:'When did the pain begin?',prompt:'When did the pain begin?',response:'It started immediately when the horses squeezed me and I fell.',keywords:['when','start','begin','onset']},
        {id:'provocation',category:'opqrst',label:'What makes the pain worse or better?',prompt:'What makes the pain worse or better?',response:'Any movement, especially trying to straighten or lower the leg, makes it much worse. Keeping my knee bent and the leg supported helps.',keywords:['better','worse','provocation','movement','straighten','lower']},
        {id:'quality',category:'opqrst',label:'What does the pain feel like?',prompt:'What does the pain feel like?',response:'It feels deep and sharp inside my left hip.',keywords:['describe','quality','feel like','sharp','deep']},
        {id:'radiation',category:'opqrst',label:'Does the pain travel?',prompt:'Does the pain move or travel anywhere?',response:'It starts in my left hip and runs down my leg.',keywords:['radiate','radiation','travel','down leg']},
        {id:'severity',category:'opqrst',label:'How severe is the pain?',prompt:'How severe is the pain from 0 to 10?',response:'It’s an eight out of ten while I’m still, and worse if the leg moves.',keywords:['severe','severity','scale','0 to 10','pain number']},
        {id:'time',category:'opqrst',label:'Has the pain changed?',prompt:'Has the pain changed since the injury?',response:'It has been constant since I fell. It eases a little when the leg stays still.',keywords:['time','changed','constant','intermittent']},
        {id:'allergies',category:'background',label:'Do you have medication allergies?',prompt:'Do you have any medication allergies?',response:'No, I don’t have any medication allergies that I know of.',keywords:['allergy','allergies','allergic']},
        {id:'medications',category:'background',label:'What medications do you take?',prompt:'What medications do you take?',response:'I take Wellbutrin. That’s my only regular prescription.',keywords:['medication','medicine','meds','wellbutrin','blood thinner','anticoagulant']},
        {id:'medical_history',category:'background',label:'What medical problems do you have?',prompt:'What medical problems do you have?',response:'I’m usually pretty healthy. I don’t have any major medical problems, and I don’t take a blood thinner.',keywords:['history','medical problem','diagnosed','past medical','blood thinner']},
        {id:'last_intake',category:'events',label:'When did you last eat or drink?',prompt:'When did you last eat or drink?',response:'I ate a few hours ago, before I came out to feed the horses.',keywords:['eat','drink','last oral','intake','meal']},
        {id:'events',category:'events',label:'Tell me exactly what happened.',prompt:'Tell me exactly what happened.',response:'I was between two horses when they crowded together and squeezed me. I fell to the ground from standing. Neither horse stepped on me.',keywords:['happened','event','horse','crushed','mechanism','fell']},
        {id:'loss_consciousness',category:'events',label:'Did you hit your head or lose consciousness?',prompt:'Did you hit your head or lose consciousness?',response:'No. I stayed awake the whole time, I remember everything, and I don’t think I hit my head.',keywords:['loss consciousness','black out','passed out','head strike','remember']},
        {id:'position',category:'events',label:'Can you move or straighten the leg?',prompt:'Can you move or straighten the left leg?',response:'I can move my foot and toes, but I can’t straighten or lower the leg because the hip pain gets severe.',keywords:['move leg','straighten','lower','foot movement','position']},
        {id:'other_injuries',category:'current',label:'Does anything else hurt?',prompt:'Does anything else hurt besides your left hip and leg?',response:'No. Nothing else hurts like the hip. My head, neck, back, chest, stomach, arms, and right leg feel okay.',keywords:['anything else hurt','other pain','other injury','else hurts']},
        {id:'numbness_tingling',category:'current',label:'Any numbness or tingling?',prompt:'Do you have any numbness or tingling in the injured leg or foot?',response:'No numbness or tingling. I can feel my foot and move my toes.',keywords:['numb','numbness','tingling','pins and needles','feel foot','sensation']},
        {id:'nausea_dizziness',category:'current',label:'Any nausea or dizziness?',prompt:'Do you feel dizzy, lightheaded, or nauseated?',response:'No. I’m not dizzy, lightheaded, or sick to my stomach.',keywords:['dizzy','dizziness','lightheaded','nausea','nauseated']},
        {id:'stepped_on',category:'events',label:'Were you stepped on?',prompt:'Did either horse step on you after you fell?',response:'No. They squeezed me and knocked me down, but neither horse stepped on me.',keywords:['stepped on','horse step','trampled','hoof']},
        {id:'moved_since_injury',category:'events',label:'Have you been moved since the injury?',prompt:'Has anyone moved you or tried to straighten the leg since you fell?',response:'No. The engine crew asked me to stay still and supported me here. Nobody has tried to straighten the leg.',keywords:['moved','straightened','since injury','engine crew','before ambulance']},
        {id:'anticoagulants',category:'background',label:'Do you take blood thinners?',prompt:'Do you take any blood thinners or anticoagulants?',response:'No, I don’t take any blood thinners.',keywords:['blood thinner','anticoagulant','warfarin','coumadin','eliquis','xarelto']},
        {id:'prior_hip_leg',category:'background',label:'Previous hip or leg problems?',prompt:'Have you ever injured or had surgery on this hip or leg before?',response:'No, I haven’t had a major injury or surgery on this hip or leg before.',keywords:['previous hip','prior hip','hip surgery','leg surgery','old injury']},
        {id:'baseline_mobility',category:'background',label:'Normal mobility',prompt:'How do you normally get around? Do you use a cane or walker?',response:'I normally walk on my own without a cane or walker, and I take care of the horses every day.',keywords:['baseline mobility','normally walk','cane','walker','ambulate']}
      ]
    }
  };
